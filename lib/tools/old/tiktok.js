...
module.exports.run = async (message, args, command, bot) => {
    ...
    try {
        ...
        console.error('(tiktok): issue loading tiktokers.json');
        return message.channel.send(`if you're reading this, something went wrong when it was never supposed to go wrong`);
    }
    
    if (!message.channel.permissionsFor(message.member).has('MANAGE_MESSAGES', true) &&
    message.author.id !== config.bot_owner) {
        return message.channel.send('Error! asdfasdf')
    }
