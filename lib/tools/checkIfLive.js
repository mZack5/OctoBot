...
async function checkIfLive(bot) {
   ...
   try {
      ...
      var config = JSON.parse(fs.readFileSync(configFle));
   } catch (err) {
   ...
   for (const user in users) {
      ...
      if (room_ids[i] !== 0 && users[user].isLive === 0) {
         ...
         // we can't use an array to store the msg_id and corolate that to the
         // channel array in tiktokers.json, because discord's api will resolve
   ...
   return new Promise((resolve) => {
      ...
      for (let i = 0; i < data.length; i++) {
         try {
            ...
            console.error(`(Livestreaming): API returned an error: DataCode: ${fileExtra}`);
         }
      }
      resolve(room_ids)
   });
