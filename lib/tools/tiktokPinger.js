...
function checkIfNewVideos(bot) {
    ...
    let log = JSON.parse(fs.readFileSync('./lib/tools/log.json'));
    ...
    const arrOfPromiseData = urls.map(eachURL => request({
        ...
        fs.writeFileSync('./lib/tools/log.json', JSON.stringify(log, null, 2));
...
function importTikTokers(bot) {
    bot.channels.get('554477335395696649').fetchMessage('554488826727497728')
        .then(msg => {
            ...
            fs.writeFileSync('./lib/tools/tiktokers.json', JSON.stringify(JSON.parse(msg), null, 2));
        });
}
