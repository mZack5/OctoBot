...
module.exports.run = async (message, args, command, bot) => {
    ...
    if (args.length > 0) {
        switch (args[0].toLowerCase()) {
            ...
            case 'add':
                ...
                for (let i = 0; i < users.length; i++) {
                    if (args[1].toLowerCase() == users[i].toLowerCase()) {
                        return message.channel.send(`Error! ${args[1]} is already receiving notifications`);
                    }
                }
                ...
                message.channel.send(`Added ${args[1]} using userid: ${args[2]} and bound to channel:${message.channel.id}`);
                break;
            ...
            case 'updaterealconfig':
                message.channel.send('Updating the config using local config.json');
