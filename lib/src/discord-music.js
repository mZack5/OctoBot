 ...
 * 
 *  helloWorld: Bootlean, enables or disables the welcome message when starting the bot, defaults to false
 */
...
module.exports = function (client, options) {
  ...
  class Music {
    constructor(client, options) {
      ...
      this.loop = "false";
      this.helloWorld = (options && options.helloWorld) || false;
    }
  }
  ...
  (function () {
    ...
    if (typeof musicbot.helloWorld !== 'boolean') {
      console.log(new TypeError('helloWorld must be a boolean'))
      process.exit(1);
    };
  ...
  // Create a blank object of queues.
  let queues = {};
  ...
  // Client ready event that checks for alive messages and outputs helloWorld
  client.on("ready", () => {
    ...
    if (musicbot.helloWorld) {
      ...
      if (musicbot.disableLeave &&
        ...
        console.log(`[NOTICE] Hey, if you wanna run it with no commands, you do you and I'll do me.`);
      }
    }
  });
          ...
          ypi.playlistInfo(musicbot.youtubeKey, playid, function (playlistItems) {
            ...
            function endrun() {
              if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                ...
                if (queuedVids.length >= 11) embed.addField(`PLACEHOLDER`, `${queuedVids.length - 10} more songs`);
                embed.setColor(0x27e33d);
                msg.channel.send({ embed });
              } else {
  ...
  function volume(msg, suffix) {
    ...
    if (suffix > 200 || suffix < 0) return msg.channel.send(note('fail', 'Volume out of range(0 -> 200)!')).then((response) => {
      response.delete(5000);
    });
