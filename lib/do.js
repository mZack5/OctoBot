...
module.exports.run = async (message, args, command, bot) => {
  ...
  switch (args[0].toLowerCase()) {
    ...
    case 'stop':
      ...
      await message.channel.send('Trying to shutdown the server!').then(msg => {
        db.droplet.msg_id = msg.id;
        db.droplet.msg_chan = message.channel.id;
        fs.writeFileSync(dbFile, JSON.stringify(db, null, 2));
      });
...
async function mainStart(bot, db) {
  ...
  // latest_snap.id == snapshot id
  // latest_snap.size_gigabytes == size 
  ...
  // we create an options Object with our digitalocean server options
  ...
  // we create a droplet, and store the returned response in `info`
