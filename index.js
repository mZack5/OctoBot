...
bot.on("message", function messageRecived(message) {
  if (message.author.bot === false) {
    let messageArguments = message.content.toLowerCase().slice(bot.prefix.length).split(" ");
    let func = commands[messageArguments.shift()];
    let command = message.content.slice(bot.prefix.length).split(" ").shift().toLowerCase();
    if (message.channel.type !== 'text') return message.channel.send('fuck outta my DMs boi');
    if (message.content.startsWith(bot.prefix)) {
      if (func !== undefined) {
        /**
         console.clear();
         console.log('message.clean:     ' + message.cleanContent);
         console.log('message command:   ' + command);
         console.log('message args:      ' + messageArguments);
         */
